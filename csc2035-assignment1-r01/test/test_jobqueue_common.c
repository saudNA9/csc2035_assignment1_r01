...
static unsigned int get_field(int method, unsigned int field) {
    switch (method) {
        case M_RAND:
            field = munit_rand_int_range(1, 20); 
            break;
        case M_SAME:
            field = 1;
            break;
...
static bool fill_queue(sjq_t* sjq, test_jq_t* test_jq, int pri_method) {
    int i = 0;
    unsigned int priority = pri_method == M_DEC ? JOB_BUFFER_SIZE + 1: 0;
    
    while (!tq_is_full(test_jq)) {
        job_t j;
        priority = get_field(pri_method, priority);
    ...
    return i == test_jq->qimpl->buf_size && !tq_is_empty(test_jq)
            && tq_is_full(test_jq) && sjq->size == test_jq->qimpl->buf_size;
}

static MunitResult test_jq_ndequeue_tofull(test_jq_t* test_jq, int pri_method) {
    assert_true(jobs_initialised(test_jq->qimpl->jobs, test_jq->qimpl->buf_size,
        test_jq->qimpl->buf_size));
        
    sjq_t sjq;
    sjq_init(&sjq);
    assert_true(fill_queue(&sjq, test_jq, pri_method));
...
MunitResult test_jq_ndequeue_randpri(test_jq_t* test_jq) {
    return test_jq_ndequeue_tofull(test_jq, M_RAND);
}

MunitResult test_jq_ndequeue_samepri(test_jq_t* test_jq) {
    return test_jq_ndequeue_tofull(test_jq, M_SAME);
}

MunitResult test_jq_ndequeue_decpri(test_jq_t* test_jq) {
    return test_jq_ndequeue_tofull(test_jq, M_DEC);
}

MunitResult test_jq_ndequeue_incpri(test_jq_t* test_jq) {
    return test_jq_ndequeue_tofull(test_jq, M_INC);
}
...
MunitResult test_jq_ndequeue_inout(test_jq_t* test_jq) {
    ...
    for (i = 0; i < ops; i++) {
        ...
        job_t expected_j;
        sjob_t sj;
        set_job(&expected_j, i, i + 1, get_field(M_RAND, 0));
    ...
    for (i = 0; i < ops; i++) {
        ...
        for (int j = 0; j < munit_rand_int_range(0, 20); j++) {
            if (sjq.size != sjq.buf_size) {
                job_t job;
                set_job(&job, get_field(M_RAND, 0), get_field(M_RAND, 0), 
                        get_field(M_RAND, 0));
...
MunitResult test_jq_ndequeue_heap(test_jq_t* test_jq) {
    sjq_t sjq;
    sjq_init(&sjq);
    assert_true(fill_queue(&sjq, test_jq, M_RAND));
